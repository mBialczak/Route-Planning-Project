...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y,
  ...
  // TODO: consider if below is necessary
  start_node->h_value = CalculateHValue(start_node); // DEBUG:
  end_node->h_value = 0;                             // DEBUG:
}
...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
  ...
  for (auto neighbour : current_node->neighbors) {
    if (!(neighbour->visited)) { //  TODO: not sure if necessary
      neighbour->parent = current_node;
      // neighbour->g_value += neighbour->distance(*current_node);//CHECK:
      neighbour->g_value =
          current_node->g_value + neighbour->distance(*current_node); // CHECK:

      neighbour->h_value = CalculateHValue(neighbour);
      AddToOpen(neighbour);
      // neighbour->visited = true;         // TODO: tidy
      // open_list.emplace_back(neighbour); // TODO: tidy
    }
  }
}
...
RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
  ...
  while (current_node->parent) {
    ...
    current_node = current_node->parent; // terminating condition
  }
  path_found.emplace_back(*start_node); // need to add starting node
...
void RoutePlanner::AStarSearch() {
  ...
  while (!open_list.empty()) {
    // counter++; // DEBUG:
    // open_list.pop_back();
    // m_Model.path.emplace_back(*current_node); // TODO: RifU

    if (current_node == end_node) {
      m_Model.path = ConstructFinalPath(current_node);
      std::cout << "PATH SIZE: " << m_Model.path.size() << std::endl; // DEBUG:
      std::cout << "METRIC SCALE: " << m_Model.MetricScale()
                << std::endl; // DEBUG:
  ...
  // std::cout << "OPEN: " << open_list.size() << std::endl; // DEBUG:
}